...
__device__ static inline void load_async_multicast(bf16 *dst, void const* const src_tma_map, uint64_t* bar, int tile_row_idx, int tile_col_idx, uint16_t cluster_mask) {
    uint64_t tma_ptr  = reinterpret_cast<uint64_t>(src_tma_map);
    uint32_t mbar_ptr = static_cast<uint32_t>(__cvta_generic_to_shared(bar));
    uint32_t dst_ptr  = static_cast<uint32_t>(__cvta_generic_to_shared(dst));
    ...
    asm volatile (
        ...
        :
        : "r"(dst_ptr), "l"(tma_ptr), "r"(mbar_ptr),
        "r"(tile_row_idx), "r"(tile_col_idx), "h"(cluster_mask)
        : "memory"
    );
}
...
__global__  __launch_bounds__(NUM_THREADS) void  __cluster_dims__(CLUSTER_M * CLUSTER_N, 1, 1) matmulKernel7(int M, int N, int K, bf16* C, const __grid_constant__ CUtensorMap tensorMapA, const __grid_constant__ CUtensorMap tensorMapB) {
    ...
    Schedule<1, NUM_SM/CLUSTERS, BLOCK_M*CLUSTER_M, BLOCK_N*CLUSTER_N, 16/CLUSTER_M, 8/CLUSTER_N> schedule(M, N, rank);
    ...
    if (wg_idx == 0) {
        ...
        if (tid == 0) {
            ...
            for (int num_block = schedule.next(); num_block >= 0; num_block = schedule.next()) {
                int num_block_n = num_block % (N / (BLOCK_N*CLUSTER_N));
                int num_block_m = num_block / (N / (BLOCK_N*CLUSTER_N));
...
void runKernel7(int M, int N, int K, bf16 *A, bf16 *B, bf16 *C, int *DB) {
    ...
    auto* kernel = matmulKernel7<BLOCK_M, BLOCK_N, BLOCK_K, NUM_THREADS, STAGES, NUM_SM, CLUSTER_M, CLUSTER_N>;
